...
func _on_JumpUpgradeButton_button_up():
	if player_node.jump_upgrade_number < 10 and player_node.coins >= 3 + int(pow(player_node.jump_upgrade_number+1, 3)):
...
func _on_HealthUpgradeButton_button_up():
	if player_node.hp_upgrade_number < 5 and player_node.coins >= 3 + int(pow(player_node.hp_upgrade_number, 5)):
		player_node.coins -= 3 + int(pow(player_node.hp_upgrade_number, 5))
		player_node.hp_upgrade_number += 1
...
func _process(_delta):
	...
	if player_node.hp_upgrade_number < 5 and player_node.coins < 3 + int(pow(player_node.hp_upgrade_number, 5)):
		$UpgradeScreen/Panel/UpgradeContainer/HealthUpgrade/VBoxContainer2/HealthUpgradeButton.disabled = true
	else: 
		$UpgradeScreen/Panel/UpgradeContainer/HealthUpgrade/VBoxContainer2/HealthUpgradeButton.disabled = false
	
	
	$UpgradeScreen/Panel/UpgradeContainer/JumpUpgrade/VBoxContainer3/Label.text = str(player_node.jump_upgrade_number) + "/10"
	...
	$UpgradeScreen/Panel/UpgradeContainer/HealthUpgrade/VBoxContainer3/Label4.text = str(player_node.hp_upgrade_number) + "/5"
	...
	$UpgradeScreen/Panel/UpgradeContainer/HealthUpgrade/Price/PriceLabel/Label.text = "$" + str(3 + int(pow(player_node.hp_upgrade_number, 5)))
